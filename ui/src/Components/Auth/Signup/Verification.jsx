...
const Verification = (props) => {
  ...
  let VerifyOtp = async () => {
    ...
    let data = {
      ...
      phoneNumber: `+${923220698313}`,
    };
    

    try {
      const res = await axios.post(
        "https://gn9y85je18.execute-api.us-west-1.amazonaws.com/dev/api/user/register/otp/verify",
        data
  ...
  const handleChange = (index, e) => {
    ...
    setVerifyOpt((previous) => ({
      ...previous,
      otp: previous.otp.substring(0, index) + value + previous.otp.substring(index + 1),
    }));
  ...
  return (
    ...
    <section className="grid lg:grid-cols-2 gap-x-[260px]">
      ...
      {/* <button onClick={VerifyOtp}>Verify OTP</button> */}
        ...
        <div className="flex  space-x-1 mt-5 px-4 lg:px-0"> <button  onClick={VerifyOtp} className=" w-[100%] justify-center py-2 flex items-center rounded-lg bg-blue-600  gap-x-2 text-white">  <img src={openlock} className="w-5 h-4"/> Continue </button></div>
      </div>
